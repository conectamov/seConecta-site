...
function leaveExplorar() { 
  explorarTimer = setTimeout(() => { showExplorar.value = false }, 180)
}
  ...
  <header v-if="!isHidden" class="w-full z-50 top-0">
    <div class="max-w-[1280px] mx-auto px-4 md:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div class="relative" data-menu @mouseenter="hoverProgramas" @mouseleave="leaveProgramas">
          ...
          <Transition enter-from-class="opacity-0 -translate-y-1 scale-95" enter-active-class="transition-all duration-150 origin-top" leave-to-class="opacity-0 -translate-y-1 scale-95" leave-active-class="transition-all duration-100 origin-top">
            <div v-if="showProgramas" class="absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 w-56 bg-white border border-[#e8e4dc] rounded-xl shadow-xl py-1.5 z-50">
              ...
              <button class="w-full text-left px-4 py-2.5 text-[0.83rem] text-gray-700 hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2" @click="go('embaixadores')">
                Embaixadores
              </button>
            </div>
          </Transition>
      ...
      <div class="flex items-center gap-1 md:gap-2">
        ...
        <div class="relative hidden lg:block" data-menu @mouseenter="hoverExplorar" @mouseleave="leaveExplorar">
          <button class="flex items-center gap-1.5 text-[0.85rem] font-semibold text-[#111] px-3.5 py-2 rounded-lg hover:bg-[#f7f5f0] transition-colors border-none bg-transparent cursor-pointer" :class="showExplorar && 'bg-[#f7f5f0] text-[#079272]'" @click.stop="toggleExplorar">
            Explorar
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" :class="['transition-transform duration-200', showExplorar ? 'rotate-180' : '']"><polyline points="6 9 12 15 18 9"/></svg>
          </button>
          <Transition enter-from-class="opacity-0 -translate-y-1 scale-95" enter-active-class="transition-all duration-150 origin-top" leave-to-class="opacity-0 -translate-y-1 scale-95" leave-active-class="transition-all duration-100 origin-top">
            <div v-if="showExplorar" class="absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 w-56 bg-white border border-[#e8e4dc] rounded-xl shadow-xl py-1.5 z-50">
              ...
              <button class="w-full text-left px-4 py-2.5 text-[0.83rem] text-gray-700 hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2" @click="go('feed')">
