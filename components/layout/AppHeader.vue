...
let explorarTimer: ReturnType<typeof setTimeout> | null = null

function closeAll() {
  showProgramas.value = false
  showExplorar.value = false
...
onUnmounted(() => {
  ...
  if (explorarTimer) clearTimeout(explorarTimer)
})
...
function hoverExplorar() {
  if (explorarTimer) clearTimeout(explorarTimer)
  showExplorar.value = true
}

function leaveExplorar() {
  if (explorarTimer) clearTimeout(explorarTimer)
  explorarTimer = setTimeout(() => {
    showExplorar.value = false
  }, 160)
}

function toggleExplorar() {
  showExplorar.value = !showExplorar.value
  showProgramas.value = false
  showUserMenu.value = false
  showNotifs.value = false
  showSocialInMobile.value = false
...
function toggleSocialInMobile() {
  showSocialInMobile.value = !showSocialInMobile.value
  showUserMenu.value = false
  showNotifs.value = false
  showProgramas.value = false
  showExplorar.value = false
  ...
  <header v-if="!isHidden" class="w-full z-50 top-0">
    <div class="max-w-[1280px] mx-auto px-4 md:px-8 h-16 flex items-center justify-between gap-4">
      ...
      <nav class="hidden lg:flex items-center gap-1">
        <div class="relative" data-menu @mouseenter="hoverExplorar" @mouseleave="leaveExplorar">
          <button
            ...
            :class="showExplorar && 'bg-[#f7f5f0] text-[#079272]'"
            @click.stop="toggleExplorar"
          >
            Explorar
            <svg
              width="12"
              ...
              stroke="currentColor"
              stroke-width="2.5"
              :class="['transition-transform duration-200', showExplorar ? 'rotate-180' : '']"
            >
          ...
          >
            <div v-if="showExplorar" class="absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 w-56 bg-white border border-[#e8e4dc] rounded-xl shadow-xl py-1.5 z-50">
              <div class="absolute -top-[5px] left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-white border-t border-l border-[#e8e4dc] rotate-45"></div>

              <button
                class="w-full text-left px-4 py-2.5 text-[0.83rem] text-gray-700 hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2"
                @click="go('feed'); showExplorar = false"
              >
              ...
              <button
                class="w-full text-left px-4 py-2.5 text-[0.83rem] text-gray-700 hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2"
                @click="go('olimpiadas'); showExplorar = false"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <button
                class="w-full text-left px-4 py-2.5 text-[0.83rem] text-gray-700 hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2"
                @click="go('calendar'); showExplorar = false"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="relative" data-menu @mouseenter="hoverProgramas" @mouseleave="leaveProgramas">
          ...
          >
            <div v-if="showProgramas" class="absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 w-56 bg-white border border-[#e8e4dc] rounded-xl shadow-xl py-1.5 z-50">
              ...
              <button
                class="w-full text-left px-4 py-2.5 text-[0.83rem] text-gray-700 hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2"
                @click="go('olimpiadas'); showProgramas = false"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M12 2L2 7l10 5 10-5-10-5z" />
                  <path d="M2 17l10 5 10-5" />
              ...
              <button
                class="w-full text-left px-4 py-2.5 text-[0.83rem] text-gray-700 hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2"
                @click="go('calendar'); showProgramas = false"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                  <line x1="16" y1="2" x2="16" y2="6" />
      ...
      <div class="flex items-center gap-1 md:gap-2">
            ...
            <div v-if="showNotifs" class="absolute right-0 top-[calc(100%+8px)] w-[340px] max-w-[calc(100vw-2rem)] bg-white border border-[#e8e4dc] rounded-2xl shadow-xl z-50 overflow-hidden">
              ...
              <div class="max-h-[360px] overflow-y-auto">
                ...
                >
                  ...
                  >
                    <svg v-if="notifMeta(notif.type).icon === 'heart'" width="13" height="13" viewBox="0 0 24 24" fill="none" :stroke="notifMeta(notif.type).color" stroke-width="2">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-[0.8rem] leading-snug" :class="notif.read ? 'text-[#666]' : 'text-[#111] font-medium'">
                      {{ notif.message }}
                    </p>
        ...
        <div class="relative hidden lg:block" data-menu>
          ...
          >
            ...
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#aaa" stroke-width="2.5" :class="['transition-transform duration-200', showUserMenu ? 'rotate-180' : '']">
              <polyline points="6 9 12 15 18 9" />
            </svg>
          </button>
          ...
          >
            <div v-if="showUserMenu" class="absolute right-0 top-[calc(100%+8px)] w-56 bg-white border border-[#e8e4dc] rounded-xl shadow-lg py-1.5 z-50">
              ...
              <button
                ...
                @click="go('perfil'); showUserMenu = false"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="hidden lg:flex items-center gap-1 mr-2">
          <a
            ...
            class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#f7f5f0] transition-all duration-200 group"
            :title="social.name"
          >
        ...
        <div class="flex items-center lg:hidden gap-1">
          <button
            class="relative w-9 h-9 flex items-center justify-center rounded-xl hover:bg-[#f7f5f0] transition-colors border-none bg-transparent cursor-pointer"
            @click.stop="toggleSocialInMobile"
            data-social
          >
          ...
          <button
            class="w-9 h-9 flex flex-col items-center justify-center gap-[5px] rounded-lg hover:bg-[#f7f5f0] transition-colors border-none bg-transparent cursor-pointer flex-shrink-0"
          ...
          >
            <div v-if="showSocialInMobile" class="absolute right-16 top-14 bg-white border border-[#e8e4dc] rounded-xl shadow-lg py-2 px-1 z-50 flex gap-1" data-social>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                class="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-[#f7f5f0] transition-all duration-200 group"
                :title="social.name"
                @click="showSocialInMobile = false"
              >
      ...
      <aside v-if="isMobileOpen" class="lg:hidden fixed top-0 right-0 h-full w-72 bg-white shadow-2xl z-50 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto">
          <nav class="px-3 py-3 space-y-0.5">
            <button
              @click="showExplorar = !showExplorar"
            ...
            <Transition
              ...
              leave-active-class="transition-all duration-100 origin-top"
            >
              <div v-if="showExplorar" class="ml-3 pl-3 border-l border-[#f1ede5] space-y-0.5">
                <button @click="go('feed')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#555] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <rect x="3" y="3" width="7" height="7" />
                    <rect x="14" y="3" width="7" height="7" />
                    <rect x="14" y="14" width="7" height="7" />
                ...
                <button @click="go('olimpiadas')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#555] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M12 2L2 7l10 5 10-5-10-5z" />
                    <path d="M2 17l10 5 10-5" />
                    <path d="M2 12l10 5 10-5" />
                ...
                <button @click="go('calendar')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#555] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                    <line x1="16" y1="2" x2="16" y2="6" />
                    <line x1="8" y1="2" x2="8" y2="6" />
            ...
            <button @click="go('sobre')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#555] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
              Sobre
            </button>

            <button @click="go('embaixadores')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#555] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
              Embaixadores
            </button>

            <button @click="go('equipe')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#555] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
              Equipe
            </button>
          </nav>
        </div>
