...
describe("canonical Student administration UX", () => {
  const list = readFileSync(new URL("./admin-users.tsx", import.meta.url), "utf8");
  const detail = readFileSync(new URL("./student-workspace.tsx", import.meta.url), "utf8");
  ...
  it("keeps technical identifiers outside the human overview", () => {
    const overview = detail.slice(detail.indexOf('tab === "overview"'), detail.indexOf('tab === "profile"'));
