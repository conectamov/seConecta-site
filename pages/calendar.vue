        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex gap-2">
            ...
            <button
              @click="scrollCarousel(1)"
              class="w-8 h-8 rounded-full border border-[#e8e4dc] bg-white flex items-center justify-center hover:border-[#079272] hover:text-[#079272] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              :disabled="carouselIndex >= recommendedPosts.length - carouselItemsPerView"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="9 18 15 12 9 6"/>
              </svg>
            ...
            >
              ...
              <div class="p-4">
                <div class="flex items-center justify-between mb-2">
                  <span v-if="post.similarity" class="text-[0.6rem] font-semibold px-2 py-0.5 rounded-full bg-[#f0faf7] text-[#079272] border border-[#c5e8df]">
                    {{ Math.round(post.similarity * 100) }}% match
                  </span>
                  <div class="flex gap-1">
                    <span v-for="tag in (post.tags || []).slice(0, 2)" :key="tag"
                      class="text-[0.55rem] px-1.5 py-0.5 bg-[#f7f5f0] text-[#888] rounded-full">#{{ tag }}</span>
                  </div>
                </div>
