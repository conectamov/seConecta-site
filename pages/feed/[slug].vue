...
const isMyPost = computed(() => currentUser.value?.id === props.post.author_id)
...
const authorName = computed(() => {
  if (isMyPost.value) {
    return currentUser.value?.full_name || currentUser.value?.username || 'Você'
  }
...
// New computed property for author username (used for profile link)
const authorUsername = computed(() => {
  if (isMyPost.value) {
    return currentUser.value?.username
  }
  return postAuthor.value?.username
})
...
const deadlineInfo = computed(() => {
  if (!props.post.deadline) return null

  const deadlineDate = new Date(props.post.deadline)
  if (isNaN(deadlineDate.getTime())) return null

  const now = Date.now()
  const diffDays = Math.ceil((deadlineDate.getTime() - now) / 86_400_000)

  return {
    date: deadlineDate,
    expired: deadlineDate.getTime() < now,
    imminent: diffDays >= 0 && diffDays <= 3,
    label: deadlineDate.toLocaleDateString('pt-BR', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
      hour: '2-digit',
    ...
    shortLabel: deadlineDate.toLocaleDateString('pt-BR', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
    }),
...
const hasDeadline = computed(() => !!deadlineInfo.value)
const isExpired = computed(() => deadlineInfo.value?.expired ?? false)

const deadlineChipText = computed(() => {
  if (!deadlineInfo.value) return ''
  if (deadlineInfo.value.expired) return 'Oportunidade encerrada'
  if (deadlineInfo.value.diffDays === 0) return 'Termina hoje'
  if (deadlineInfo.value.diffDays === 1) return 'Termina amanhã'
  if (deadlineInfo.value.imminent) return `Termina em ${deadlineInfo.value.diffDays} dias`
  return `Deadline: ${deadlineInfo.value.shortLabel}`
})
...
const deadlineCardClass = computed(() => {
  if (!deadlineInfo.value) return 'bg-[#f7f5f0] text-[#888] border-[#e8e4dc]'
  if (deadlineInfo.value.expired) return 'bg-red-50 text-red-700 border-red-200'
  if (deadlineInfo.value.imminent) return 'bg-amber-50 text-amber-700 border-amber-200'
  return 'bg-[#eef8f4] text-[#079272] border-[#d6ede5]'
})
...
// New function to navigate to author profile
  ...
  >
    <div class="p-7 md:p-8 flex flex-col">
      <div class="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div
          class="w-7 h-7 flex-shrink-0 rounded-full overflow-hidden cursor-pointer"
        ...
        <div class="flex items-center gap-2 text-[0.7rem] text-[#bbb] flex-wrap">
          ...
          <span
            class="text-[0.75rem] font-medium text-[#555] cursor-pointer hover:text-[#079272] transition-colors"
            @click.stop="goToAuthorProfile"
          >
            {{ authorName }}
          </span>
          ...
          <span
            v-if="hasDeadline"
            class="flex items-center gap-1 px-2 py-0.5 rounded-full border text-[0.68rem] font-semibold"
            :class="deadlineChipText ? deadlineCardClass : ''"
          >
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <rect x="3" y="4" width="18" height="18" rx="2" />
              <line x1="16" y1="2" x2="16" y2="6" />
        ...
        <span
          ...
          class="ml-auto text-[0.62rem] font-semibold px-2 py-0.5 rounded-full bg-amber-50 border border-amber-200 text-amber-600"
        >
          Em análise
        </span>
      ...
      <p v-if="post.excerpt" class="text-sm font-light text-[#777] leading-relaxed line-clamp-2 mb-4">
        {{ post.excerpt }}
      </p>
      ...
      <div v-if="visibleTags.length > 0" class="flex flex-wrap gap-1.5 mb-4">
        <span
          ...
          :key="tag"
          class="text-[0.65rem] font-semibold px-2 py-0.5 bg-[#f7f5f0] border border-[#e8e4dc] text-[#888] rounded-full"
        >
          #{{ tag }}
        </span>
        <span v-if="(post.tags?.length ?? 0) > 4" class="text-[0.65rem] text-[#bbb]">
          +{{ post.tags.length - 4 }}
        </span>
      </div>
      ...
      <div class="flex items-center justify-between mt-auto pt-4 border-t border-[#f5f3f0]">
        <div class="flex items-center gap-0.5">
          <button
            class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[0.75rem] text-[#bbb] border-none bg-transparent cursor-pointer transition-all hover:bg-[#f7f5f0] hover:text-[#333]"
          ...
          <button
            class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[0.75rem] border-none bg-transparent cursor-pointer transition-all hover:bg-[#f7f5f0]"
            :class="saved ? 'text-[#079272]' : 'text-[#bbb]'"
        ...
        <button
          class="flex items-center gap-1.5 px-4 py-2 bg-[#0d0d0d] text-white rounded-lg text-[0.75rem] font-semibold border-none cursor-pointer transition-all hover:bg-[#079272]"
          @click.stop="emit('read', post)"
        >
          Ler
