...
const { post: apiPost, get: apiGet } = useAxios()
...
onMounted(async () => {
  const saved = localStorage.getItem('new-guide-draft')
  if (saved) {
    try {
      const parsed = JSON.parse(saved)
      if (parsed.title || parsed.description_md) {
...
watch(form, (val) => {
  localStorage.setItem('new-guide-draft', JSON.stringify(val))
}, { deep: true })

async function loadGuides() {
  loadingGuides.value = true
  try {
    const res = await apiGet('/guides/?skip=0&limit=200')
...
function discardDraft() {
  form.value = { title: '', description_md: '', excerpt: '', cover_url: '', slug: '', parent_guide_id: null, tags: [] }
...
async function handleSubmit() {
  ...
  if (form.value.parent_guide_id !== null) payload.parent_guide_id = form.value.parent_guide_id
  ...
  try {
    ...
    if (status === 401 || status === 403) error.value = 'Sem permissão para criar guias.'
    else if (status === 422) error.value = 'Dados inválidos. Verifique os campos.'
    else error.value = 'Erro ao criar o guia. Tente novamente.'
  } finally {
    ...
    <main class="flex-1 min-w-0">
      ...
      <Transition name="slide-fade">
        <div v-if="success" class="mb-6 p-5 rounded-2xl bg-[#f0faf7] border border-[#079272]/20 flex items-start gap-3">
          ...
          <div>
            <p class="font-bold text-[#079272] text-[0.88rem]">Guia criado com sucesso!</p>
            <p class="text-[0.8rem] text-[#0DA790] mt-0.5">O guia foi enviado para análise. Redirecionando…</p>
          </div>
        </div>
      </Transition>
      ...
      <div class="bg-white rounded-2xl border border-[#e8e4dc] shadow-sm overflow-hidden">
        <div class="px-6 pt-6 pb-4 border-b border-[#f7f5f0] flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 class="text-[1.1rem] font-extrabold text-[#111] tracking-[-0.02em]">Novo guia</h1>
            <p class="text-[0.75rem] text-[#aaa] mt-0.5">Estruture conteúdos e trilhas para a comunidade</p>
          </div>
        ...
        <div class="p-6 space-y-5">
          ...
          <div>
            <label class="block text-[0.78rem] font-semibold text-[#555] mb-1.5">Resumo <span class="text-[#bbb] font-normal">(opcional — útil para listagens)</span></label>
            <textarea
          ...
          <div>
            ...
            <p class="text-[0.7rem] text-[#aaa] mt-1">Se vazio, o sistema pode gerar automaticamente no backend.</p>
          </div>
